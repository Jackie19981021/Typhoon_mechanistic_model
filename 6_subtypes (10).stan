...
generated quantities {
  ...
  // MODIFIED: 47 scenarios for detailed analysis
  ...
  // MODIFIED: New typhoon scenario parameters
  // 47 scenarios: 1 baseline + 40 (4 durations × 10 intensities) + 6 (shifted timing, excluding 0)
  ...
  // MODIFIED: Extended scenarios (281 total: 1 baseline + 280 combinations)
  // 4 durations × 10 intensities × 7 shifts = 280
  ...
  // Define scenario parameters
  {
    ...
    // Main scenarios: 4 durations × 10 intensities (indices 2-41)
    // Intensities: -50%, -30%, -20%, -10%, -5%, +5%, +10%, +20%, +30%, +50%
    ...
    // Shifted timing scenarios: 7 days duration, -50% intensity, shifts except 0 (indices 42-47)
  ...
  for (typhoon_idx in 1:47) {
    real typhoon_change = typhoon_effects[typhoon_idx];  // Can be negative (reduction) or positive (increase)
    ...
    if (shift >= 0) {
      ...
      for (t_rel in 1:T_days_forecast) {
        ...
        // Modification factor: 1 + typhoon_change during typhoon period
