# ============================================================================
# TYPHOON IMPACT ANALYSIS - EXTENDED WITH MODIFIED SCENARIOS
...
cat("Note: Now computing 47 main scenarios + 281 extended scenarios\n")
cat("Main scenarios: 1 baseline + 40 (4 durations × 10 intensities) + 6 (shifted timing)\n")
cat("Extended scenarios: 1 baseline + 280 (4 × 10 × 7)\n\n")
...
#fit <- readRDS("/Users/chenjiaqi/Desktop/COVID-19_HK/typhoon/typhoon_model_fit_modified.rds")
...
cat("Number of forecast points per strain per scenario:", 
    nrow(typhoon_forecast_df) / (N_strains * 47), "(includes overlap point)\n\n")
...
# ============================================================================
# FIGURE 2: Recent Period + Selected Typhoon Scenarios + Validation Data
# ============================================================================
...
# 3 days scenarios (reduction and increase)
...
# ============================================================================
# FIGURE 10: Typhoon Impact Effectiveness - TYPHOON PERIOD ONLY
# ============================================================================
cat("\n=== Creating Figure 10: Typhoon Impact Effectiveness (Typhoon Period Only) ===\n")

# Extract reduction for TYPHOON PERIOD ONLY
...
p11_3days <- create_avg_cases_plot(selected_3d_red_indices, 
                                   "Average Weekly Cases: Typhoon vs Recovery Period - 3 Days Duration")
print(p11_3days)
...
p11_7days <- create_avg_cases_plot(selected_7d_red_indices, 
                                   "Average Weekly Cases: Typhoon vs Recovery Period - 7 Days Duration")
print(p11_7days)
ggsave("figure11_avg_weekly_cases_7days.png", p11_7days, width = 14, height = 10, dpi = 300, bg = "white")

p11_10days <- create_avg_cases_plot(selected_10d_red_indices, 
                                    "Average Weekly Cases: Typhoon vs Recovery Period - 10 Days Duration")
print(p11_10days)
ggsave("figure11_avg_weekly_cases_10days.png", p11_10days, width = 14, height = 10, dpi = 300, bg = "white")

p11_14days <- create_avg_cases_plot(selected_14d_red_indices, 
                                    "Average Weekly Cases: Typhoon vs Recovery Period - 14 Days Duration")
print(p11_14days)
ggsave("figure11_avg_weekly_cases_14days.png", p11_14days, width = 14, height = 10, dpi = 300, bg = "white")

p11_shifted <- create_avg_cases_plot(selected_shifted_red_indices, 
                                     "Average Weekly Cases: Typhoon vs Recovery Period - Shifted Timing Scenarios")
print(p11_shifted)
...
cat("(Data from typhoon period was NOT used in model fitting)\n")
cat(rep("=", 100), "\n", sep="")
...
write.csv(validation_comparison,
          file = "/Users/chenjiaqi/Desktop/COVID-19_HK/typhoon/validation_comparison_modified.csv",
          row.names = FALSE)
...
attack_rate_df$scenario <- factor(attack_rate_df$scenario, levels = c("0% (Baseline)", typhoon_scenarios))
...
# ============================================================================
# FIGURE 15A: COMBINED SUNBURST CHART - All 6 strains in one figure
# Using manual approach for independent color scales per strain
# ============================================================================
cat("\n=== Creating Figure 15A: Combined Sunburst Chart (6 strains, 2x3 layout) ===\n")
...
p15a_combined <- (p1 | p2 | p3) / (p4 | p5 | p6) +
  plot_annotation(
    title = "Typhoon Impact Scenarios: Case Incidence per 10,000 Population",
    subtitle = "Inner: Duration (3d/7d/10d/14d) | Middle: Intensity (-50% to +50%) | Outer: Timing (E=Early, D=Delayed, On=On time)\nExtension length = relative incidence within intensity group | Color: blue = lower, red = higher | Each strain has independent color scale",
    theme = theme(
...
print(p15a_combined)
ggsave(
  filename = "figure15a_sunburst_combined_6strains.png",
  plot = p15a_combined,
  width = 20,
  height = 13,
  dpi = 300,
...
cat("\nSaved: figure15a_sunburst_combined_6strains.png\n")

# ============================================================================
# FIGURE 15B: Heatmap Grid for all scenarios
# ============================================================================
cat("\n=== Creating Figure 15B: Reduction Heatmap ===\n")
...
# Reorder intensity factor
grid_df$intensity <- factor(grid_df$intensity, 
                            levels = c("-50%", "-30%", "-20%", "-10%", "-5%", 
                                       "5%", "10%", "20%", "30%", "50%"))

p15b_grid <- ggplot(grid_df, aes(x = shift, y = intensity, fill = reduction)) +
  geom_tile(color = "white", size = 0.5) +
  geom_text(aes(label = sprintf("%.0f", reduction)), size = 1.8, color = "black") +
  ...
  scale_fill_gradient2(low = "#D6604D", mid = "white", high = "#4393C3",
                       midpoint = 0,
                       name = "Change\n(%)",
                       limits = c(min(grid_df$reduction), max(grid_df$reduction))) +
  scale_x_discrete(labels = c("-7" = "E7", "-5" = "E5", "-3" = "E3", "0" = "On",
                              "3" = "D3", "5" = "D5", "7" = "D7")) +
  labs(
    title = "Typhoon Impact: Case Change by Duration, Intensity, and Timing",
    subtitle = "Values show percentage change in cases during typhoon period (compared to baseline)\nPositive = reduction | Negative = increase | Columns: Strains | Rows: Duration | X: Timing | Y: Intensity",
    x = "Timing Shift (days)",
...
print(p15b_grid)
ggsave("figure15b_reduction_heatmap.png", p15b_grid,
       width = 16, height = 10, dpi = 300, bg = "white")

cat("\n=== Figure 15 Complete ===\n")
cat("Generated files:\n")
cat("  Combined sunburst (1 file):\n")
cat("    - figure15a_sunburst_combined_6strains.png (2×3 layout, each with independent color bar)\n")
cat("  Heatmap grid (1 file):\n")
cat("    - figure15b_reduction_heatmap.png\n")
cat("\n=== All Figure 15 visualizations complete ===\n")
...
cat("\nModified Scenario Configuration:\n")
cat("  - Intensities: -50%, -30%, -20%, -10%, -5%, +5%, +10%, +20%, +30%, +50%\n")
cat("  - Durations: 3, 7, 10, 14 days\n")
cat("  - Timing shifts: -7, -5, -3, 0, +3, +5, +7 days (for -50% intensity only)\n")
cat("  - Total main scenarios: 47 (1 baseline + 40 duration×intensity + 6 shifted)\n")
cat("  - Total extended scenarios: 281 (1 baseline + 280 combinations)\n")

cat("\nGenerated Figures:\n")
cat("  Figure 1: Historical fit + baseline forecast + validation\n")
cat("  Figure 2: Recent period + scenarios (4 duration + 1 shifted)\n")
cat("  Figures 3-6: Baseline vs typhoon forecasts (4 duration + 1 shifted)\n")
cat("  Figure 7: Baseline R_eff\n")
cat("  Figure 8: R_eff by scenario (4 duration + 1 shifted)\n")
cat("  Figure 9: Change in R_eff from baseline (4 duration + 1 shifted)\n")
cat("  Figure 10: Typhoon impact effectiveness (4 duration + 1 shifted)\n")
cat("  Figure 11: Average weekly cases (4 duration + 1 shifted)\n")
cat("  Figure 12: Validation comparison\n")
cat("  Figure 13: Attack rate analysis (NEW - comparison and differences)\n")
cat("  Figure 14: Posterior distributions\n")
cat("  Figure 15: Sunburst and heatmap visualizations\n")

cat("\nGenerated Tables:\n")
cat("  - validation_comparison_modified.csv\n")
cat("  - attack_rate_summary.csv (NEW)\n")

cat("\nKey Outputs:\n")
cat("  - All scenarios include both transmission reduction AND increase\n")
cat("  - Attack rates calculated up to T_weeks - 2 (before typhoon)\n")
cat("  - Continuity fixes applied to all forecast visualizations\n")
cat("  - Independent color scales for each strain in sunburst\n")
